...
#RUNS SUCCESSFULLY ON : environment_vectorised_cost_compute.py
import tensorflow as tf
import numpy as np
from environment_vectorised_operations import InventoryEnv
import time
np.random.seed(2)
...
class Actor(object):
    ...
    def choose_action(self, s):
        s = s[np.newaxis, :]
        a = self.sess.run(self.action, {self.s: np.squeeze(s,2)})  # get probabilities for all actions
...
actor = Actor(sess, n_features=N_S, n_outputs = N_O,lr=LR_A, action_bound=[-A_BOUND, A_BOUND])
